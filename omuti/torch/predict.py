...
from omuti.torch import preprocess
...
def predict_file(model, csv_file, savedir, device, iou_threshold=0.1, color=(0, 165, 255), thickness=1):
    ...
    for index, prediction in enumerate(prediction_list):
        ...
        if savedir:
            ...
            image = np.array(Image.open(paths[index]))[:, :, ::-1]
