...
def plot_predictions(image, df, color=None, thickness=1):
    ...
    if image.shape[0] == 3:
        raise ValueError(
            "Input images must be channels last format [h, w, 3] not channels first [3, h, w], use np.rollaxis(image, 0, 3) to invert")
    ...
    for index, row in df.iterrows():
        ...
        cv2.rectangle(image, (int(row["xmin"]), int(row["ymin"])), (int(row["xmax"]), int(row["ymax"])), color=color,
